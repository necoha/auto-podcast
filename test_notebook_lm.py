...
def test_notebook_lm_access():
    ...
    try:
        # オートメーター初期化（GitHub Actions環境ではマルチブラウザ自動化を含む）
        automator = OAuthNotebookLMAutomator()
        
        # マルチブラウザ自動化の確認
        if os.getenv('GITHUB_ACTIONS') and hasattr(automator, 'multi_browser_automator'):
            if automator.multi_browser_automator and automator.multi_browser_automator.driver:
                browser_info = automator.multi_browser_automator.get_browser_info()
                print(f"✅ {browser_info['browser'].capitalize()}ブラウザで初期化成功")
            else:
                print("⚠️ マルチブラウザ自動化の初期化に失敗しました")
