...
> **正本と実装状態**: 本書を要求の正本とし、HLD・LLD・コードを順に合わせる。以下の事実照合・配信品質ゲートは目標仕様であり、現行の見出し限定配信にはまだ実装されていない。
...
- 重複排除後の最大20件を検証候補とし、全候補への言及は必須としない
- URL Contextを最大5件ずつのバッチで取得し、URLごとに引用付き事実カードを作成する
- 出典の内容で主張を裏付けられた記事だけを採用し、要約と重要事実を解説する。背景・影響は元記事で確認できる場合に限り扱い、空欄を推測で埋めない
- 取得・裏付けに失敗した記事は放送対象から除外し、成功済み記事は保持する。見出しだけの紹介で穴埋めしない
- 採用記事の事実カードから対話形式の台本を構築する。採用記事が0件なら、その回を生成・配信しない
- 英語の記事タイトルは、元の意味を増減させず聞き取れる日本語に言い換える。未翻訳の英文をそのまま読み上げない
- 根拠のある内容だけで5〜8分の速報を構成できない場合は、その回を配信しない

### FR-02-D: ポッドキャストスクリプト生成（深掘り版）
- 重複排除後の候補から、タイトル・媒体名だけで重要度の高い最大3件を先に選定する
- 選定した最大3件だけをURL Contextと台本生成へ渡し、6つの分析次元（背景・経緯、技術解説、業界影響、異なる視点、日本と海外の比較、今後の展望）で深掘りする
- 10〜15分の対話形式（3000〜5000文字）、1トピックあたり5〜8往復の議論
- 裏付けのある記事と主張だけで10〜15分の分析を構成できない場合は、短い見出し版を代替配信しない
- 速報版と同じ PRONUNCIATION_MAP（発音辞書）を共有する

### FR-02-V: 台本の事実検証
- 目的は、元記事で裏付けられない主張を確かなニュースとして放送しないこと。URL取得成功とAIの引用注釈は根拠候補であり、それだけで事実確認済みとはしない
- 速報版は生成前にURL Contextで事実カードを作り、各主張を同一URLに紐づく元記事の内容で照合する
- 深掘り版は生成後に選定済み記事の元記事内容と台本の主張を照合し、主張ごとの根拠URLを特定する
- 数値、年月、価格、割合、法律・規制・制度、主体と指標の対応が元記事と一致することを確認する。単なる引用文字範囲の重なりは一致の証明としない
- 中央銀行の政策金利と民間銀行の預金金利など、同じ単位を持つ別指標を混同しない
- 深掘り版では、検証できない背景・因果関係・海外比較を追加しない
- 元記事本文・該当箇所にアクセスできない場合や、引用はあっても主張の整合性を確認できない場合は未検証として扱う
- 速報版は検証不能な記事を除外し、深掘り版は台本・選定記事の事実確認に失敗したら、その回を生成・配信しない。いずれも見出し限定回へは切り替えない
- 採用記事0件でも、元記事を確認した結果の裏付け不足と、503・429などで確認処理自体ができなかった障害は区別する。どちらも配信しないが、後者を品質不合格として報告しない
- 元記事との照合結果、採用・除外理由、根拠URLをエピソードメタデータと実行ログで追跡できるようにする

### FR-03: 音声生成（TTS）
- スクリプトから音声ファイルを自動生成する
- 自然で聞きやすい音声品質を実現する
- 複数話者による対話形式（Multi-Speaker TTS、20行単位のチャンク）をサポートする
- 14人の話者が曜日ごとにローテーション（7ペア）し、速報版と深掘り版で同じペアを使用する
- 放送日・読み上げ内容が台本と一致し、長い無音、冒頭からの読み直し、発話の欠落がないことを配信前に確認する。放送日は日本時間で統一する

### FR-04: 配信
- リスナーがポッドキャストアプリ（Apple Podcasts, Spotify等）で購読できる
- 速報版と深掘り版をそれぞれ独立したRSSフィード（`feed.xml` / `feed_deep.xml`）で配信する
- 新エピソードは事実の裏付け・台本の分量と読みやすさ・生成音声の品質に合格した場合だけ自動配信する。RSS XMLの構造検証だけでは配信許可としない
- 一方の番組が不合格でも他方の生成・配信を続ける。不合格の番組は音声・RSS項目を追加せず、既存フィードを維持する
...
- 手動実行にも対応する
- 番組ごとに「配信済み」「品質不合格で配信見送り」「実行障害」を区別し、見送りを配信成功として報告しない
...
- 国内6ソース（テクノロジー）+ 海外3ソース（テクノロジー）+ 国内4ソース（経済）= 計13ソース
- 1フィードあたり最大2記事を取得し、重複排除後に公開日時が新しい全体20記事へ絞る

#### 4.1.1 RSSニュースソース採用・除外基準

RSS/Atomフィードが技術的に取得できることだけでは採用しない。候補は以下の必須条件をすべて満たすこと。

| 観点 | 採用条件 |
|------|----------|
| 利用条件 | 媒体の利用規約・RSS利用条件を確認し、自動取得・AIによる二次生成・配信を明示的に禁止していないこと。不明確でリスクを解消できない場合は採用しない |
| 編集適合性 | 一般リスナーが背景知識なしで理解できるテクノロジーまたは経済の時事ニュースが中心であること。開発者・研究者向けの専門解説、連載・チュートリアル・広告記事が中心の媒体は採用しない |
| 技術適合性 | 認証なしで取得できる公開RSS/Atomで、`feedparser`がタイトル・媒体名・URL・公開日時を安定して解析できること |
| 更新性 | 原則として直近24時間の記事を継続的に取得できること。低頻度でもセキュリティ勧告など速報価値が高い場合は例外とする |
| 構成バランス | 既存媒体との記事重複、言語・分野の偏り、1日あたりの記事総数を評価し、既存ソースで十分にカバーできる媒体は追加しない |
| 著作権対策 | LLMへ渡す情報は記事タイトル・媒体名・URLに限定し、RSSのsummaryや記事本文をプロンプトへ含めない現行方針を維持できること |

`config.RSS_FEEDS`を承認済みソースのallowlistとする。審査未了の媒体は追加せず、環境変数`CUSTOM_RSS_FEEDS`で一時追加する場合にも同じ基準を適用する。

採用または再評価時は、利用規約のURL・確認日・判断理由、RSSの疎通結果、24時間内の記事数をIssueまたはコミットへ記録する。利用規約や配信内容が変わった場合は再審査する。
...
| **B. Google Gemini Flash** | 無料枠あり | ◎ | ◎ | 3.8を優先し、一時障害時はURL Context対応のStable 3.7 / 3.6へ切替 |
...
|------|-------------|------|---------|--------|------|
| **A. Gemini 3.1 Flash TTS** | **無料** | ◎ | ◎ プロンプト指示 | ◯ | `gemini-3.1-flash-tts-preview` 無料枠あり。Multi-Speaker・ストリーミング対応 |
...
**推奨: A（Gemini 3.1 Flash TTS）** — 無料・高品質・対話形式対応・プロンプトで制御可能 ⭐
...
|---------|------|--------|
| コンテンツ収集 | RSS フィード | $0 |
| スクリプト生成 | Gemini 3.8 / 3.7 / 3.6 Flash（無料枠） | $0 |
| 音声生成 | Gemini 3.1 Flash TTS（無料枠） | $0 |
