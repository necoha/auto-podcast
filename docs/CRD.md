...
> **現行実装（2026-09-26）**: 既定モデルはGemini 3.8 Flash / 3.1 Flash TTS Preview、定期実行は23:00 JST。台本は記事タイトル・媒体名・URLから生成し、元記事本文との事実照合は行わない。以下のモデル・料金比較と06:00 JSTの記載は当初の構想であり、現行のAPI利用料金はアカウントのプランによる。
