...
- 重複排除後の最大20件を速報対象として維持する
- URL Contextを最大5件ずつのバッチで取得し、URLごとに引用付き事実カードを作成する
- 取得・引用成功記事は要約、重要事実、背景、影響を解説する
- 取得・引用失敗記事はタイトル・媒体名だけを紹介し、成功済みカードを失わない
- 事実カードと見出しから対話形式の台本を決定論的に構築する
- 日本語対応
...
### FR-02-V: 台本の事実検証
- 速報版は生成前にURL Contextで事実カードを作り、各カードの全事実文字列へ同一URLの引用範囲を要求する
- 深掘り版は生成後に選定済み最大3 URLで台本を照合する
- 数値、年月、価格、割合、法律・規制・制度、主体と指標の対応を元記事の引用証跡と照合する
- 中央銀行の政策金利と民間銀行の預金金利など、同じ単位を持つ別指標を混同しない
- 深掘り版では、検証できない背景・因果関係・海外比較を追加しない
- 高リスク主張に引用範囲がない場合は台本を不合格とする
- 速報版は検証不能な記事だけを見出し紹介へ切り替え、深掘り版は未検証台本を破棄して見出し限定へ切り替える
- 検証状態と参照URLをエピソードメタデータへ保存する
